...
const boxGenerator = document.getElementById('box-generator');
...
function updateWaveBoxes(note, isPlaying) {
    const waveBox = document.createElement('div');
    waveBox.classList.add('wave-box');

    if (isPlaying) {
        boxGenerator.appendChild(waveBox);
    } else {
        const existingBox = Array.from(boxGenerator.children).find(box => box.classList.contains('wave-box'));
        if (existingBox) {
            boxGenerator.removeChild(existingBox);
        }
    }
}
