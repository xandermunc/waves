...
function drawParametricCurve(svg, funcX, funcY, tMin, tMax, steps, scaleX, scaleY, translateX, translateY) {
    ...
    for (let i = 0; i <= steps; i++) { // this is what draws the points and connects them with straight lines. maybe updating this will fix the detail issue
...
// function getJustIntonationInterval(note) {
//     const justIntonationRatios = {
//         'F': 1, 'F#': 16 / 15, 'G': 9 / 8, 'G#': 6 / 5, 'A': 5 / 4,
//         'A#': 4 / 3, 'B': 45 / 32, 'C': 3 / 2, 'C#': 8 / 5,
//         'D': 5 / 3, 'D#': 9 / 5, 'E': 15 / 8
//     };
//     return justIntonationRatios[note] || 1;
// }
...
function animate() {
    ...
    drawParametricCurve(svgCanvas,
        t => intervals.reduce((sum, interval) => sum + Math.sin(interval * t + phi), 0), // x
        // t => intervals.reduce((sum, interval) => sum + Math.cos(interval * t + phi), 0), // comment out y (the line below) and uncomment this to see top view
        t => B * Math.cos(t), // y
        0, 2 * bInterval * Math.PI, 1000, // bInterval is the LCM of the denominators of each JI interval 
        200 / B, 200 / B, 250, 250); // removing B changes the scale based on the number of notes (this didn't work on zoom for some reason)
